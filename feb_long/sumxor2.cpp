...
inline base operator * (base a, base b) {
    return base(a.x*b.x - a.y*b.y, a.x*b.x + a.y*b.y);
}
...
vector<int> multiply(vector<int> &a, vector<int> &b, int eq = 0) {
    ...
    for(int i=0; i<= (sz>>1); i++) {
        ...
        if(i!=j) {
            ...
            base d1 = (B[j] + conj(B[j]))*r3, d2 = (B[j] - conj(B[i]))*r4;
            A[i] = c1*d1 + c2*d2*r5;
            B[i] = c1*d2 + c2*d1;
        }
        A[j] = a1*b1 + a2*b2*r5;
...
struct poly {
    ...
    pair<poly, poly> divmod_slow(const poly &b) const {
        vector<mint> A(a);vector<mint> ans;
...
int po(int a, int deg){
    ...
    int t = po(a, deg/2);return mul(t, t);
}
int inv(int n){
    return po(n, p-2);
}
...
int C(int n, int k){
    if (n<k) return 0;return mul(facs[n], mul(invfacs[k], invfacs[n-k]));
}

int fac(int x){
    if (x==0) return 1;
    int cnt = x/p;
    int res = mul(facs[x%p], fac(cnt));
    if (cnt%2==1) res = sub(0, res);
    return res;
}
...
int32_t main() {
    ...
    while(q--){
        ...
        int ans=0;
        for(int i=0;i<32;i++){
            ans+=(nbit[m][i]%md*(1ll<<i)%md)%md;ans%=md;
        }
        cout<<ans<<endl;
    }
    return 0;
